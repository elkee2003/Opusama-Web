...
const HotelSearch = () => {
  ...
  const [nightlifePosts, setNightlifePosts] = useState([]);
  const [filteredData, setFilteredData] = useState([]);

  const handleSearch = () => {
    if (!nightlifePosts || nightlifePosts.length === 0) {
      return;
    }
    ...
    const filtered = nightlifePosts.filter((item) => {
      const matchesQuery =
  ...
  const fetchRealtorsAndPosts = async () => {
    try {
      ...
      const nightlifePostData = filteredPosts.map((post) => {
      ...
      setNightlifePosts(nightlifePostData);
      setFilteredData(nightlifePostData);
    } catch (error) {
  ...
  useEffect(() => {
    handleSearch();
  }, [searchQuery, minPrice, maxPrice, nightlifePosts]);
