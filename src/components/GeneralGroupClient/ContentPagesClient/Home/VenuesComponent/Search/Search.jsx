...
const HotelSearch = () => {
  ...
  const handleSearch = () => {
    ...
    const filtered = venuePosts.filter((item) => {
      const matchesQuery =
  ...
  const fetchRealtorsAndPosts = async () => {
    try {
      ...
      const venuePostData = filteredPosts.map((post) => {
      ...
      setVenuePosts(venuePostData);
      setFilteredData(venuePostData);
    } catch (error) {
  ...
  useEffect(() => {
    handleSearch();
  }, [searchQuery, minPrice, maxPrice, venuePosts]);
