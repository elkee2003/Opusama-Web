...
const CommercialSpaceSearch = () => {
  const [searchQuery, setSearchQuery] = useState('');
  const [minPrice, setMinPrice] = useState('');
  const [maxPrice, setMaxPrice] = useState('');
  const [commercialSpacePosts, setCommercialSpacePosts] = useState([]);
  const [filteredData, setFilteredData] = useState([]);

  const handleSearch = () => {
    if (!commercialSpacePosts || commercialSpacePosts.length === 0) {
      return;
    }
    ...
    const filtered = commercialSpacePosts.filter((item) => {
      const matchesQuery =
  ...
  const fetchRealtorsAndPosts = async () => {
    try {
      ...
      setCommercialSpacePosts(commercialSpacePostData);
      setFilteredData(commercialSpacePostData);
    } catch (error) {
  ...
  useEffect(() => {
    handleSearch();
  }, [searchQuery, minPrice, maxPrice, commercialSpacePosts]);
...
export default CommercialSpaceSearch;