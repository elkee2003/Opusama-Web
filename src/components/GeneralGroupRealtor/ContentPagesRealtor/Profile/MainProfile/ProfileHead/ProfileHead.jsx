...
const ProfileHead = () => {
    ...
    const { dbRealtor, userMail } = useAuthContext();
    ...
    // ✅ Automatically save realtor email if missing
    useEffect(() => {
        const saveEmailIfMissing = async () => {
        try {
            // Only run if dbRealtor exists and has no email yet
            if (dbRealtor && !dbRealtor.email && userMail) {
