...
const ViewMedia = () => {
    ...
    const onUpdateSliderChange = (values, handle) => {
        setVideoTrimmedUrl(''); // Reset trimmed video when adjusting
    ...
    const handleTrim = async () => {
        ...
        try {
            const videoBlob = await fetch(mediaItem.uri).then(res => res.blob());
        ...
        {mediaItem.type === "video" ? (
            <div>
                ...
                <div className="trim-function">
                    ...
                    <p>Start: {formatTime(sliderValues[0])} | End: {formatTime(sliderValues[1])}</p>

                    <button 
                        onClick={handleTrim} 
                        disabled={!isFFmpegLoaded}
                        className="trim-btn"
                    >{isTrimming ? "Trimming..." : "Trim Video"}</button>
                </div>
            </div>
        ) : (
            <div className="mediaWrapper">
