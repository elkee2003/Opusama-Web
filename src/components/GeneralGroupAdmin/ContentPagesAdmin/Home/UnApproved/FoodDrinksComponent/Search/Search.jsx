...
const HotelSearch = () => {
  ...
  const [foodDrinksPosts, setFoodDrinksPosts] = useState([]);
  const [filteredData, setFilteredData] = useState([]);

  const handleSearch = () => {
    if (!foodDrinksPosts || foodDrinksPosts.length === 0) {
      return;
    }
    ...
    const filtered = foodDrinksPosts.filter((item) => {
      const matchesQuery =
  ...
  const fetchRealtorsAndPosts = async () => {
    try {
      ...
      const sortedPosts = foodDrinksPostData.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
      );

      setFoodDrinksPosts(sortedPosts);
  ...
  useEffect(() => {
    handleSearch();
  }, [searchQuery, minPrice, maxPrice, foodDrinksPosts]);
