...
const HotelSearch = () => {
  ...
  const handleSearch = () => {
    if (!propertySalePosts || propertySalePosts.length === 0) {
      return;
    }
  ...
  const fetchRealtorsAndPosts = async () => {
    try {
      ...
      const filteredPosts = posts.filter((post) => post.propertyType === 'House Sale' || post.propertyType === 'Land Sale');
      ...
      const sortedPosts = propertySalePostData.sort(
        (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
      );

      setPropertySalePosts(sortedPosts);
